...
def extract_values_from_text(text: str) -> dict:
    ...
    for line in lines:
        ...
        for test_name in REFERENCE_RANGES:
            if test_name in lower_line:
                numbers = re.findall(r'[-+]?\d*\.?\d+', line)
                if numbers:
                    try:
                        value = float(numbers[0])
                        if test_name not in extracted:
                            extracted[test_name] = {'value': value, 'raw': line.strip()}
                    except ValueError:
                        continue
    return extracted
...
def flag_values(extracted: dict, gender: str = 'all') -> list:
    ...
    for test_name, data in extracted.items():
        ...
        unit      = ref.get('unit', '')
        full_name = ref.get('full_name', test_name.title())
...
def run_rule_engine(text: str, gender: str = 'all') -> dict:
    ...
    return {'extracted_values': extracted, 'flagged_items': flags, 'severity': severity}