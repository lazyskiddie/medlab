...
def generate_pdf_report(analysis_result) -> str | None:
    ...
    sev_color = {
        'normal':   colors.HexColor('#1D9E75'),
        'mild':     colors.HexColor('#BA7517'),
        'moderate': colors.HexColor('#D85A30'),
        'severe':   colors.HexColor('#E24B4A'),
    ...
    conditions = analysis_result.conditions_detected
    if conditions:
        story.append(Paragraph('Conditions Detected', heading_style))
        for c in conditions:
            story.append(Paragraph(
                f"• <b>{c['name']}</b> — confidence: {c.get('confidence', 'n/a')}",
                normal_style,
            ))
        story.append(Spacer(1, 0.6*cm))
    ...
    if flags:
        ...
        header = ['Test', 'Value', 'Unit', 'Reference Range', 'Status']
        rows   = [header]
        for f in flags:
            rows.append([
                f['test'], str(f['value']), f['unit'],
                f"{f['low']} – {f['high']}",
        ...
        for i, f in enumerate(flags, start=1):
            if f['is_critical']:
                bg, fg = colors.HexColor('#FCEBEB'), colors.HexColor('#A32D2D')
            elif f['status'] == 'high':
                bg, fg = colors.HexColor('#FAEEDA'), colors.HexColor('#633806')
            elif f['status'] == 'low':
                bg, fg = colors.HexColor('#E6F1FB'), colors.HexColor('#0C447C')
            else:
                bg, fg = colors.HexColor('#EAF3DE'), colors.HexColor('#27500A')
            style_cmds += [
    ...
    disc_style = ParagraphStyle('disc', parent=normal_style, fontSize=8,
                                 textColor=colors.HexColor('#888780'))
    story.append(Paragraph(
        'This report is generated by an automated system for informational purposes only. '
