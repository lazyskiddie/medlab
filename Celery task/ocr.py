...
def preprocess_image(image_array: np.ndarray) -> np.ndarray:
    """
    Applies a sequence of image cleanup steps to maximise OCR accuracy:
    ...
    # Adaptive threshold (handles uneven lighting common in phone photos)
...
def ocr_from_pdf(file_path: str) -> str:
    ...
    1. Try pdfplumber for text-based PDFs (fast, accurate).
    2. Fall back to image conversion + Tesseract for scanned PDFs.
    """
    texts = []
...
def extract_text(file_path: str, file_type: str) -> str:
    """Main entry point — dispatches to image or PDF OCR."""
    path = str(file_path)
    if file_type == 'pdf':
